...
{
	public static Vector2 GetScreenCentre() => ImGuiHelpers.MainViewport.GetCenter();

	// offset is L/B/R/T, added directly to the position of each side of viewport
	public static bool IsScreenPosInsideMainViewport(Vector2 screenPos, Vector4 offset)
	{
	...
	public static Vector3 TranslateNormalisedCoordinates(Vector3 pos3norm, bool reverseY = true)
	{
		var mSizeHalf = ImGuiHelpers.MainViewport.Size / 2;
		var mCentrePos = ImGuiHelpers.MainViewport.Pos + mSizeHalf;
		return new(
			mCentrePos.X + pos3norm.X * mSizeHalf.X,
			reverseY
	...
	public static Vector2 GetConstrainedScreenPos(
		Vector2 screenPos,
		Vector4 screenConstraint,
		Vector2 extraConstraint
	)
	{
		var constraintUL = ImGuiHelpers.MainViewport.Pos + extraConstraint;
		var constraintBR =
			ImGuiHelpers.MainViewport.Pos + ImGuiHelpers.MainViewport.Size - extraConstraint;
		var x = Math.Clamp(
			screenPos.X,
			constraintUL.X + screenConstraint.X,
			constraintBR.X - screenConstraint.Z
		);
		var y = Math.Clamp(
			screenPos.Y,
			constraintUL.Y + screenConstraint.W,
			constraintBR.Y - screenConstraint.Y
		);
		return new(x, y);
	}
	...
	)
	{
		var constraintUL =
			...
			+ new Vector2(screenConstraint.X, screenConstraint.W)
			+ extraConstraint;
		var constraintBR =
			ImGuiHelpers.MainViewport.Pos
			+ ImGuiHelpers.MainViewport.Size
			- new Vector2(screenConstraint.Z, screenConstraint.Y)
			- extraConstraint;
	...
	public static float GetAngleOnScreen(Vector2 origin, Vector2 point, bool flipped = false) =>
		flipped
			? MathF.Atan2(origin.X - point.X, origin.Y - point.Y)
	...
	{
		// TODO:  ColorConvertRGBtoHSV does not correct return out
		//maybe it will be noticed
		return new Vector4(0, 0, 0, colour.W);
	...
	public static Vector2 GetTextSize(string text, ImFontPtr font, float fontsize)
	{
		...
		return new(maxLineW * fontsize / font.FontSize, fontsize * lineCount);
	}
	...
	)
	{
		...
		var shadowCol_uint = ImGui.ColorConvertFloat4ToU32(
			GenerateShadowColour(colour, shadowLightness)
		);
		// showdow R
		pos.X += scale;
	...
	public static void DrawMultilineTextWithShadow(
		ImDrawListPtr drawList,
		string text,
		Vector2 pos,
		ImFontPtr font,
