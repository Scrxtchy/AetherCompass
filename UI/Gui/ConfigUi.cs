...
public static class ConfigUi
{
	...
	public static void Draw()
	{
		...
		Plugin.Overlay.AddDrawAction(
			() =>
				ImGui
					.GetWindowDrawList()
					.AddRect(
						new(displayArea.X, displayArea.W),
						new(displayArea.Z, displayArea.Y),
		...
		Plugin.Overlay.AddDrawAction(
			Compass.GenerateConfigDummyMarkerDrawAction(
	...
	private static void DrawPluginSettingsTab()
	{
		// ScreenMark
		ImGuiEx.Checkbox(
			"Enable marking detected objects on screen",
			ref Plugin.Config.ShowScreenMark,
		...
		if (Plugin.Config.ShowScreenMark)
		{
			...
			ImGuiEx.DragFloat(
				"Marker size scale",
				100,
				ref Plugin.Config.ScreenMarkSizeScale,
				.01f,
				PluginConfig.ScreenMarkSizeBound.Min,
				PluginConfig.ScreenMarkSizeBound.Max
			);
			ImGuiEx.DragFloat(
				"Marker text size scale",
				100,
				ref Plugin.Config.ScreenMarkTextRelSizeScale,
				.1f,
				PluginConfig.ScreenMarkTextRelSizeBound.Min,
				PluginConfig.ScreenMarkTextRelSizeBound.Max,
				"%.1f",
			...
			var displayArea = GetDisplayAreaFromConfigScreenMarkConstraint();
			ImGuiEx.DragFloat4(
				"Marker display area (Left/Bottom/Right/Top)",
				ref displayArea,
				1,
				PluginConfig.ScreenMarkConstraintMin,
				9999,
				v_fmt: "%.0f",
			...
			Plugin.Config.ScreenMarkConstraint = new(
				...
				displayArea.W - viewport.Y
			); // U
			ImGui.Text(
				$"(* The full screen display area is "
					+ $"<{viewport.X:0}, {viewport.Y + vsize.Y:0}, {viewport.X + vsize.X:0}, {viewport.Y:0}> )"
			);
			ImGuiEx.Checkbox(
				"Hide marker when nameplate is inside the marker display area",
			...
			if (Plugin.Config.HideScreenMarkIfNameplateInsideDisplayArea)
			{
				...
				ImGuiEx.DragInt(
					...
					Language.Unit.Yalm,
					50,
					ref Plugin.Config.HideScreenMarkEnabledDistance,
					1,
		...
		// DetailWindow
		ImGuiEx.Checkbox(
			"Show detected objects' details",
			ref Plugin.Config.ShowDetailWindow,
		...
		{
			ImGuiEx.Checkbox(
				"Hide compass UI when in event",
				ref Plugin.Config.HideInEvent,
			...
			ImGuiEx.Checkbox(
				"Hide compass UI when crafting/gathering/fishing",
				ref Plugin.Config.HideWhenCraftGather
			);
		}
		ImGui.NewLine();
	...
	private static Vector4 GetDisplayAreaFromConfigScreenMarkConstraint()
	{
		...
		return new(
			...
			viewport.Y + Plugin.Config.ScreenMarkConstraint.W
		); // U
	}
}
