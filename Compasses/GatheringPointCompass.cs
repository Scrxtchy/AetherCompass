...
public class GatheringPointCompass : Compass
{
	...
	protected override CompassConfig CompassConfig => Plugin.Config.GatheringConfig;
	...
	public override bool IsEnabledInCurrentTerritory() =>
		...
		|| ZoneWatcher.CurrentTerritoryType?.TerritoryIntendedUse.ValueNullable?.RowId
			== 47 // diadem
	;
	...
	public override DrawAction? CreateMarkScreenAction(CachedCompassObjective objective)
	{
		...
		var descr =
			$"{GetGatheringLevelDescription(objective.DataId)} {objective.Name}, "
	...
	private static uint GetGatheringPointIconId(
		byte gatheringType,
		GatheringPointType gatheringPointType
	)
	{
		...
		if (typeRow == null)
			return 0;
		return (uint)(
			IsSpecialGatheringPointType(gatheringPointType)
				? typeRow.Value.IconOff
				: typeRow.Value.IconMain
		);
	}

	private static uint GetGatheringPointIconId(uint dataId)
	{
		var gatherType = GatheringPointSheet
			?.GetRow(dataId)
			.GatheringPointBase.ValueNullable?.GatheringType.Value;
		if (gatherType == null)
			return 0;
		return (uint)(
			IsSpecialGatheringPoint(dataId) ? gatherType.Value.IconOff : gatherType.Value.IconMain
		);
	}
	...
	{
		exportedGatheringPointRowIds = GatheringPointSheet
			?.Where(p => p.TerritoryType.Row == terr)
			.Select(p => p.GatheringPointBase.Row);
	}

	private void DrawUiForTerritoryExportedGatheringPoints()
	{
		if (exportedGatheringPointRowIds == null)
			return;
		if (GatheringPointBaseSheet == null || ExportedGatheringPointSheet == null)
			return;
		foreach (var rowId in exportedGatheringPointRowIds)
		{
			...
			if (gatherPointBase == null || exported == null || exported.GatheringPointType == 0)
				continue;
			var pos = new Vector3(exported.X, exported.Y, 0);
			var name =
				$"(Nodes Group: Lv{gatherPointBase!.GatheringLevel} "
				+ InferGatheringPointName(
					exported.GatheringType,
					(GatheringPointType)exported.GatheringPointType
				)
				+ ")";
			...
			if (MarkScreen)
				overlay.AddDrawAction(
					new(
						() =>
						{
							var icon = IconManager.GetGatheringMarkerIcon(
								GetGatheringPointIconId(
									exported.GatheringType,
									(GatheringPointType)exported.GatheringPointType
								)
							);
							string descr =
								$"{name}, {CompassUtil.DistanceToDescriptiveString(dist, true)}";
							DrawScreenMarkerDefault(
								pos,
								0,
								icon,
				...
				detailsWindow.AddDrawAction(
					...
					new(
						() =>
						{
							ImGui.Text(name);
							ImGui.BulletText(
								$"{CompassUtil.GetMapCoordInCurrentMapFormattedString(pos, false)} (approx.)"
							);
							ImGui.BulletText(
								$"{CompassUtil.GetDirectionFromPlayer(pos)},  "
									+ $"{CompassUtil.DistanceToDescriptiveString(dist, false)}"
							);
							ImGui.BulletText(CompassUtil.GetAltitudeDiffFromPlayerDescriptive(pos));
							DrawFlagButton(
								$"##NodesGroup_{rowId}",
								CompassUtil.GetMapCoordInCurrentMap(pos)
							);
							ImGui.Separator();
						},
						dist < 200
	...
	private static string InferGatheringPointName(
		byte gatheringType,
		GatheringPointType gatheringPointType
	) =>
		GatheringPointNameSheet
			?.GetRow(
	...
	#endregion FUTURE/ ExportedGatheringPoint
}
